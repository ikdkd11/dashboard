...
bar1r = result1.bar1
data1 = {
    "순서": ["첫 번째 저온구간", "두 번째 저온구간", "세 번째 저온구간-1", "세 번째 저온구간-2", "네 번째 저온구간", "다섯 번째 저온구간"],
...
st.header('■ 분석 결과2 - 공통 저온구간 선정 및 특성', divider='rainbow')
st.markdown("""
    <style>
    .big-font {
    ...
    <div class="big-font">
     공통 저온구간이란? 1~4차 관측에서 노면온도가 감소하는 정도가 크고 그 감소 양상이 일관되게 유지되는 구간들을 공통 저온구간으로 산출<br>
     공통 저온구간의 경우, 다리와 터널 혹은 다리와 다리가 연쇄적으로 이어져있는 구조로 이로 인해 노면온도가 연달아서 감소하여 다른 곳에 비해 결빙 가능성이 높음
    </div>
    """, unsafe_allow_html=True)
st.subheader(' ')
st.subheader('공통 저온구간 5곳 상세 정보')
st.table(df1)
st.markdown('')
st.subheader('공통 저온구간의 연쇄 노면 냉각')
st.markdown("""
    <style>
    .big-font {
        font-size:20px !important;
...
with col1:
    option = st.selectbox('표시 이미지 선택:',
                 ['위성 사진으로 보는 네 번째 저온구간에서의 노면온도 감소 지점',
                  '노면온도 그래프로 보는 세풍대교와 초남1교에 의한 연쇄 냉각효과',
                  '세풍대교 사진',
                  '초남1교 사진'
                  ])
with col2:
    if option == '위성 사진으로 보는 네 번째 저온구간에서의 노면온도 감소 지점':
        st.image(image1)
...
st.markdown("""
    ...
    <div class="big-font">
     > 이러한 연쇄 노면냉각이 극대화 되는 지점이 다섯 번째 저온구간이었음<br>
