...
st.header('저온구간 세번째 - 벌교대교', divider='rainbow')    
...
with col2:
    if option == '저온구간3(벌교대교-장양육교) 위성사진':
...
st.subheader('저온구간3(벌교대교-장양육교) 1~4차 관측회차 별 시계열 그래프 및 박스그림                                                                               ')        
col1, col2 = st.columns([1,1])
...
with col2:
    ...
    with tab1:
       st.subheader('저온구간(벌교대교) 중 최저 노면온도 기록구간 별 노면온도 비교')
