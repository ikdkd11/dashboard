...
st.header('저온구간 네번째 - 세풍대교', divider='rainbow')    
...
with col2:
    if option == '저온구간4(세풍대교-초남1교-초남터널) 위성사진':
...
st.subheader('저온구간4(세풍대교-초남1교-초남터널) 1~4차 관측회차 별 시계열 그래프 및 박스그림                                                                               ')
col1, col2 = st.columns([1,1])
col1.plotly_chart(grph4, use_container_width = True)
with col2:
    tab1, tab2, tab3 = st.tabs(["Table", "Graph1", "Graph2"])
    with tab1:
       st.subheader('저온구간(세풍대교) 중 최저 노면온도 기록구간 진입 전/후 평균 노면온도 비교')
